...
class HeatmapImageGenerator(object):
    ...
    def generate_heatmap(self, coords, sigma_scale_factor, dtype=np.float32):
        ...
        :return: dd array of the landmark image.
        """
    ...
    def generate_heatmaps(self, landmarks, stack_axis, dtype=np.float32):
        ...
        for landmark in landmarks:
            if not np.array_equal(landmark, [0,0]) : 
            # .is_valid:
                heatmap_list.append(self.generate_heatmap(landmark, 1, dtype))
            else:
...
# HMG_128 = HeatmapImageGenerator((128,128), 2, 30)
# heatmaps_all = []
# for i in range(2000):
#     heatmaps = HMG_128.generate_heatmaps(np.reshape(coor_array_np_128[i], (6,2)),0)
#     heatmaps_all.append(heatmaps)
# heatmaps_all = np.asarray(heatmaps_all)
# heatmaps_all = np.reshape(heatmaps_all, (2000, 6, 128,128))